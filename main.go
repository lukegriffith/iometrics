package main

import (
	"bufio"
	"flag"
	"fmt"
	"log"
	"os"
	"time"
)

var size = flag.Int("size", 8, "file size in GiB")

func main() {

	// get pvc size
	// get mount location
	// get block size

	// while total written less than pvcsize
	// write file of block size to mount location

	// attempt write run until error

	// delete all and return

	// RETURN MEAN, MAX, MIN. RETURN IF FAILED SINCE LAST CALL.

	for true {
		measureIO()
	}
}

func measureIO() {

	flag.Parse()

	fSize := int64(*size) * (1024 * 1024 * 1024)
	err := writeFile(fSize)
	if err != nil {
		fmt.Fprintln(os.Stderr, fSize, err)
	}
}

func timeTrack(start time.Time, name string) {
	elapsed := time.Since(start)
	log.Printf("%s took %s", name, elapsed)
}

func writeFile(fSize int64) error {

	defer timeTrack(time.Now(), "main")
	fName := `C:\\Users\\lukem\\Documents\blah` // test file
	defer os.Remove(fName)
	f, err := os.Create(fName)
	if err != nil {
		return err
	}
	const defaultBufSize = 4096
	buf := make([]byte, defaultBufSize)
	buf[len(buf)-1] = '\n'
	w := bufio.NewWriterSize(f, len(buf))

	start := time.Now()
	written := int64(0)
	for i := int64(0); i < fSize; i += int64(len(buf)) {
		nn, err := w.Write(buf)
		written += int64(nn)
		if err != nil {
			return err
		}
	}
	err = w.Flush()
	if err != nil {
		return err
	}
	err = f.Sync()
	if err != nil {
		return err
	}
	since := time.Since(start)

	err = f.Close()
	if err != nil {
		return err
	}
	fmt.Printf("written: %dB %dns %.2fGB %.2fs %.2fMB/s\n",
		written, since,
		float64(written)/1000000000, float64(since)/float64(time.Second),
		(float64(written)/1000000)/(float64(since)/float64(time.Second)),
	)
	return nil
}
